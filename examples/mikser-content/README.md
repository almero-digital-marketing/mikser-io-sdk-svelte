...
A standalone mikser project. Run it once, point all three Svelte examples at it. They share one catalog, three rendering shapes.
...
| `schemas/*.js` | Zod schemas — one per layout (`page`, `article`, `product`, `landing`, `slot`). `mikser-io-plugin-schemas` validates every loaded entity against the schema that matches its `meta.layout`, and emits `entities.d.ts` for the Svelte apps to import as types. |
...
| Layout | Where it appears | Required meta fields | Svelte view |
|---|---|---|---|
...
| `slot` | `slots/*.md` | + starts_at, duration_minutes, available | (no view — surfaces inside `BookingForm.svelte` island only) |

The body is plain Markdown; mikser's `render-markdown` plugin converts it to HTML for the catalog. The Svelte apps consume that HTML via `document.content` and render it with `{@html document.content}`.
...
Then in each of the Svelte example projects:
...
# Terminal 3 — hybrid (SvelteKit dev)
cd examples/hybrid-ssg && npm run dev
...
1. Open all three Svelte examples in your browser at once.
...
- `pure-spa` re-renders `ProductView.svelte` — button enables, label flips
- `hybrid-ssg`'s `/admin` editor does the same (the public, prerendered build doesn't, by design — that's the point of the hybrid split)
- `islands` — if you wire the `cart-counter` to surface stock state, that updates too

Same `useDocument` reactive on the client. Same SSE event from the server. Same file on disk as the source of truth.
...
- **`meta.layout`** — drives the per-layout dispatch (which view component renders this document). Match the keys in each example's `route-mapping.js`.
- **`meta.href`** — the logical reference (same across all translations).
- **`meta.route`** — the deployed URL (different per translation). It's what shows in the address bar; `useHref` returns it.
...
The `hybrid-ssg` build (SvelteKit prerender via `generateMikserRoutes` inside an `entries()` hook) reads from the api at build time, then doesn't talk to the server again. The editor route talks to the api at runtime and stays subscribed.
...
Try adding a new file under `documents/en/articles/` while the examples are running. It appears at the top of `ArticleIndex.svelte` in `pure-spa` and `hybrid-ssg`'s editor — no manual refresh.
...
1. **Validates each entity's `meta` against the matching schema** as it's loaded. The demo runs in `onError: 'warn'` mode, so a typo (e.g. dropping `author` from an article) shows up as a server log line — the entity still appears in the catalog and the Svelte apps still render it. Flip to `onError: 'fail'` in `mikser.config.js` to make the build exit non-zero on the first invalid entity.
2. **Emits `entities.d.ts`** at the project root after every build, exposing one `XxxMeta` type per layout plus a `MetaByLayout<L>` helper. The Svelte apps import from this file to get typed `useDocument` results:

   ```ts
   import type { MetaByLayout } from '../mikser-content/entities'
   import { useDocument } from 'mikser-io-sdk-svelte'

   const article = useDocument<{ meta: MetaByLayout<'article'> }>(() => entityId)
   //                                                                ↑
